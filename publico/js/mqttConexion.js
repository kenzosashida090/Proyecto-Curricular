...
export const borrarEcg = async (id) => {
  try {
    const res = await axios({
      method: "DELETE",
      url: `http://127.0.0.1:3000/api/graficas/borrar-grafica/${id}`,
    });
    if (res.data.status === "exitoso") {
      mostrarAlerta("exitoso", "Se elimino correctamente!");
      window.setTimeout(() => {
        location.assign("/grafica");
      }, 1500);
    }
    ...
    console.log(err);
    mostrarAlerta("error", "Algo salio borrando los datos!");
  }
};
