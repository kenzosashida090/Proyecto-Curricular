...
import { borrarEcg, pubECG } from "./mqttConexion";
let suscrito = false;
...
const borrarBtn = document.querySelector("#delete-button");
const btnPub = document.querySelector("#btnPECG");
const borrarPaciente = document.querySelector("#deleteP");

const idEcg = document.querySelector("#iDe");
...
const canvasEcg = document.getElementById("chartEcg");
...
if (borrarPaciente) {
  const id = document.querySelector("#pacID").innerHTML;
  borrarPaciente.addEventListener("click", () => {
    deletePaciente(id);
  });
}
if (borrarBtn) {
  const id = idEcg.innerHTML;
...
if (canvasEcg) {
  let tiempo = document.getElementById("tiempoEcg").innerHTML;
  let ecgData = document.getElementById("ecgDf").innerHTML;
  const arrayTiempo = tiempo.split(",");
  const arrayEcg = ecgData.split(",");
  const ctx = canvasEcg.getContext("2d");
  const chartEcg = new Chart(ctx, {
    type: "line", // Tipo de gráfico (puede ser "line", "bar", "pie", etc.)
    data: {
      labels: arrayTiempo, // Etiquetas de los datos en el eje X
      datasets: [
        {
          label: "Datos", // Etiqueta del conjunto de datos
          data: arrayEcg, // Datos a graficar
  ...
  btnLogico.addEventListener("click", () => {
    ...
    if (!suscrito) {
      ...
      cliente.onMessageArrived = (mensaje) => {
        ...
        let valor = parseFloat(mensaje.payloadString);
        ecgDato.push(valor);
