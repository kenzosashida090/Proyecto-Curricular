...
exports.getEcg = catchAsync(async (req, res, next) => {
  const idPaciente = req.params.pID;
  const ecgInfo = await Ecg.find().populate("paciente");
...
exports.getGrafica = catchAsync(async (req, res, next) => {
  ...
  const ecgInfo = await Ecg.findOne({ _id: ecgId });
  console.log("hola", ecgInfo);
  if (!ecgInfo) {
    return next(new AppError("Ningun ecg se encontro con ese ID", 404));
  }
  res.status(200).render("ecg-id", {
    titulo: `ECG`,
    ecgInfo,
  });
});
